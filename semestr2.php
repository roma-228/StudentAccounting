	...
	<section class="body">
		...
		<div class="inner-wrapper">
			...
			<section role="main" class="content-body kursePanel ">
				...
				<script>
					<?php
					//Блок групи
					$KyrsePanel1 = $connect->query("SELECT COUNT(*) as COUNT FROM `subjects` WHERE `id_semestr`=$id_semestr[2] and `id_ group`=$id_group");
					while ($row = $KyrsePanel1->fetch_assoc()) {
						$num = $row["COUNT"];
					}
					$KyrsePanel1 = $connect->query("SELECT * FROM `subjects` WHERE `id_ group`=$id_group and `id_semestr`=$id_semestr[2] ORDER by `id_Subjects`");
		...
		<script>
			function but() {
				...
				<?php
				$KyrsePanel2 = $connect->query("SELECT * FROM `subjects` WHERE `id_semestr`=$id_semestr[2] and `id_ group`=$id_group");
